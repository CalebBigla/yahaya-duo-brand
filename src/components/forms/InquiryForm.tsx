    ...
    <div className="card-interactive rounded-2xl border border-border bg-card p-6 shadow-card sm:p-8">
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate className="mt-6 grid gap-4 sm:grid-cols-2">
        {fields.map((f) => {
          ...
          return (
            <div key={f.name} className={f.full || f.type === "textarea" ? "sm:col-span-2" : ""}>
              ...
              {f.type === "textarea" ? (
                <textarea id={id} rows={4} placeholder={f.placeholder} className={`${cls} input-focus`} {...register(f.name)} />
              ) : f.type === "select" ? (
                <select id={id} className={`${cls} input-focus`} {...register(f.name)} defaultValue="">
        ...
        <div className="sm:col-span-2">
          <button
            type="submit"
            className="btn-interactive inline-flex w-full items-center justify-center gap-2 rounded-full bg-accent px-6 py-3 text-sm font-bold text-accent-foreground sm:w-auto"
          >
